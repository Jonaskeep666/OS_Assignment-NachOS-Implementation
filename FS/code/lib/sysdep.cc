...
// 23-0419[j]: 本檔案提供一些 Linux / C++ 的 API(函數) 供 NachOS 使用
//             部份會直接呼叫 Linux(Host Machine) 的 System Call

// 23-0419[j]: MP1 的 File Operation 是呼叫此處的 API (並呼叫 Host System Call)
//             路徑: exception.cc -> ksyscall.h -> filesys.h -> sysdep.h -> #include <stdlib.h>
...
#else
    int pgSize = getpagesize();                 // 23-0131[j]: systemCall 取得 Host 一個 PageSize 有多大
    char *ptr = new char[pgSize * 2 + size];

    mprotect(ptr, pgSize, 0);                   // 23-0131[j]: 首位 page 設為不可 存取，以防 Overflow
