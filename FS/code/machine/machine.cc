...
//----------------------------------------------------------------------
// 23-0419[j]: mipssim.cc 實作了 Machine::Run() -> 呼叫 Machine::OneInstruction() -> 呼叫 RaiseException()
// 22-1223[j]: 引發「中斷」= 指令運算發生「特別事件」(需要例外處理)，呼叫 RaiseException(which, badVAddr)
// 22-1223[j]: 主要工作 (1) 將 DelayLoad 暫存的值處理完畢 (2) 切換至 Kernel Mode (3) 處理例外事件 (4) 切換回 User Mode
...
Machine::RaiseException(ExceptionType which, int badVAddr)
{
    ...
    // 22-1223[j]: 切換到 Kernel Mode
    kernel->interrupt->setStatus(SystemMode); 
    
    // 22-1223[j]: ExceptionHandler(事件編號) 在 exception.cc 定義
    // 22-1223[j]: Switch Case、ISR(中斷服務程式)
    ExceptionHandler(which);		// interrupts are enabled at this point 
    
    // 22-1223[j]: 切換回 User Mode
