...
// 22-1230[j]: 宣告 System Call 的函式樣貌 & 對應的 System Call ID
// 22-1230[j]: User program 要使用 System Call 服務程式，需要呼叫 宣告在此處的「函式」
// 22-1230[j]: 所以 User program 都會 #include "syscalls.h"

// 22-1230[j]: User program 呼叫以下「函式」後，會由 Start.s 將 對應 System Call ID 存入暫存器 -> 見Start.s 
...
// 23-0419[j]: System Call ID 在 start.S (SysCall Interface) & ExceptionHandler 會用到
#define SC_Halt		    0
#define SC_Exit		    1
#define SC_Exec		    2
#define SC_Join		    3
 ...
 * the console device.
 */
// 23-0419[j]: Linux 的週邊設備(含 I/O) 都可識別為一個 File ex. stdin, stdout
