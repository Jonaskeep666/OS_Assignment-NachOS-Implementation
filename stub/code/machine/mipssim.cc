...
// 23-0419[j]: 這個檔案 定義 MIPS 的指令 要對 Reg 做的運算
...
// 22-1223[j]: 指令 (32) = Operator (opCode(6)) + Operand (運算元 = rs(5), rt(5), rd(5) 三個) + Extra(11)
// 22-1223[j]: MIPS ISA 的指令 有3種型態: I、J、R 分別對應不同 Operand 個數
// 22-1223[j]: Format R = opCode(6) - rs(5) - rt(5) - rd(5) - shamt(5) - funct(6)
// 22-1223[j]: Format I = opCode(6) - rs(5) - rt(5) - immediate (16)
// 22-1223[j]: Format J = opCode(6) - address(26)
...
// 
//  // 23-0419[j]: kernel 在初始化時 呼叫 Machine::Run 且不會自行停止
//  //             此後會一直 Call 他，來模擬 CPU 的執行
//
//	This routine is re-entrant, in that it can be called multiple
//	times concurrently -- one for each thread executing user code.
//----------------------------------------------------------------------
// 22-1223[j]: Run() 
// 22-1223[j]: 主要工作: (1) 執行指令(instr) (2) 每個Tick都 中斷一次 (3) 若為 singleStep模式，則每執行完一到指令呼叫一次 Debugger()
...
Machine::Run()
{
    ...
    // 22-1223[j]: 若開啟 debug 的 machine emulation 功能，則會印出 currentThread & totalTicks 的資訊
    if (debug->IsEnabled('m')) {
        cout << "Starting program in thread: " << kernel->currentThread->getName();
	    cout << ", at time: " << kernel->stats->totalTicks << "\n";
    }
    
    // 22-1223[j]: 設定為 UserMode (需要中斷時 才切 KernelMode)
    ...
    for (;;) {
        ...
        kernel->interrupt->OneTick();	// 23-0127[j]: 模擬 時間快轉 1 Tick
...
//
// 22-1223[j]: "re-entrant": 表示「函式A」被中斷後 OS 執行另一段「程式碼B」，且該「程式碼B」又呼叫了「函式A」，整個流程不會Error
// 
//	This routine is re-entrant, in that it can be called multiple
//	times concurrently -- one for each thread executing user code.
...
//----------------------------------------------------------------------
// 22-1223[j]: OneInstruction(*instr) 主要工作: 
// 22-1228[j]: (1) [從 Reg(34)=PCReg 指向的記憶體位址 ] 讀入指令 並 Decode 
// 22-1228[j]: (2) 根據 不同 instr->opCode 對 運算元進行運算(包含 system call 中斷)
...
Machine::OneInstruction(Instruction *instr)
{
    ...
    instr->value = raw;
    instr->Decode();    // 22-1223[j]: 解碼指令 = raw = instr->value 

    // 22-1223[j]: 有開啟 debug machine simulation 的功能，就印出 執行的指令
    if (debug->IsEnabled('m')) {
    ...
    int pcAfter = registers[NextPCReg] + 4; // 22-1223[j]: 將 Program counter 指向下個 指令位址(4 byte = 32 bits) 
    int sum, diff, tmp, value;
    unsigned int rs, rt, imm;

    // Execute the instruction (cf. Kane's book)
    // 22-1223[j]: 根據 不同 instr->opCode 對 運算元進行運算
    switch (instr->opCode) {

      case OP_ADD:
	sum = registers[instr->rs] + registers[instr->rt];

    // 22-1223[j]: 若兩數相加「Overflow」-> 進入例外處理 RaiseException(OverflowException, 0) = 引發「中斷」
	...
	break;
      ...
      case OP_SYSCALL:	// 23-0419[j]: MP1 注意
...
// 22-1223[j]: DelayedLoad(MyReg, MyValue): 將 MyValue & MyReg 暫存起來，下次呼叫 DelayedLoad 時，將 MyValue 存入 MyReg暫存器中
//             首次呼叫 DelayedLoad(3, 10):將 3 存入registers[LoadReg] = nextReg，將 10 存入 registers[LoadValueReg]
//             再次呼叫 DelayedLoad(0, 0): 將 10 存入 registers[3] = register[registers[LoadReg]]
//             registers[LoadReg]: DelayLoad中，暫存 的 Reg編號 = nextReg
//             registers[LoadValueReg]: DelayLoad中，暫存 的 Value = nextValue
...
Instruction::Decode()
{
    ...
    rs = (value >> 21) & 0x1f;  // 22-1223[j]: opCode(6) + rs(5) + rt(5) + rd(5) + [shamt(5)+funct(6)]
    rt = (value >> 16) & 0x1f;
    rd = (value >> 11) & 0x1f;
    opPtr = &opTable[(value >> 26) & 0x3f]; // 22-1223[j]: 取出 opCode(6)
    opCode = opPtr->opCode;
    if (opPtr->format == IFMT) {        // 22-1223[j]: Format I = opCode(6) - rs(5) - rt(5) - immediate (16)
        extra = value & 0xffff;
        if (extra & 0x8000) {
    	   extra |= 0xffff0000;
	    }
    } else if (opPtr->format == RFMT) { // 22-1223[j]: Format R = opCode(6) - rs(5) - rt(5) - rd(5) - shamt(5) - funct(6)
	    extra = (value >> 6) & 0x1f;
    } else {                            // 22-1223[j]: Format J = opCode(6) - address(26)
